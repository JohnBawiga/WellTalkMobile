...
class MindAndBodyGym extends Component {
  ...
  getExercises = async (category, difficulty) => {
    try {
      let apiUrl = `https://welltalk-mobile-backend.up.railway.app/exercises/getByExerciseType?exercisetype=${category}`;

      if (difficulty !== 'All') {
        apiUrl = `https://welltalk-mobile-backend.up.railway.app/exercises/getByExerciseTypeAndDifficulty?exercisetype=${category}&difficulty=${difficulty}`;
      }
